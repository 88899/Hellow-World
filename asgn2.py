...
def PMI(c_xy, c_x, c_y, N):
  ...
  return PMI # you need to fix this
...
def cos_sim(v0,v1):
  ...
  for keyi in key0:
      if keyi in key1:
          dotproduct += v0[keyi]*v1[keyi]
  normal_v0 = np.linalg.norm(list(v0.values()))
  normal_v1 = np.linalg.norm(list(v1.values()))
  cos = dotproduct/(normal_v0*normal_v1)
...
def create_ppmi_vectors(wids, o_counts, co_counts, tot_count):
    ...
    for wid0 in wids:
        ...
        vectors[wid0] = {}
        for i in o_counts.keys():
            if i not in co_counts[wid0].keys():
                break
            PPMI = max(0,PMI(co_counts[wid0][i],o_counts[i],o_counts[wid0],tot_count))
            if PPMI !=0:
                vectors[wid0][i]=PPMI
    return vectors
...
print(np.array(list(o_counts.keys())).shape)
