...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    # f_t = [batch_size, num_features]
                    f_t = torch.bmm(step_attn_weights.unsqueeze(dim=1),src_embeddings.transpose(0,1)).squeeze(dim=1)
                    h_t = self.lexical_FFNN(f_t)
                    # h_t = [batch_size, num_features]
                    h_t = torch.tanh(h_t)+h_t
                    h_t = F.dropout(h_t, p= self.dropout_out ,training=self.training)
        ...
        if self.use_lexical_model:
            ...
            lexical_out = torch.cat(lexical_contexts, dim=0).view(tgt_time_steps, batch_size, self.embed_dim)
            lexical_out = lexical_out.transpose(0, 1)
            
            decoder_output += self.lexical_final_projection(lexical_out)
