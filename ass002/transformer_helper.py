...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        K_kd = self.k_proj(key).contiguous().view(-1, batch_size,self.num_heads,self.head_embed_size)
        Q_kd = self.q_proj(query).contiguous().view(tgt_time_steps, batch_size,self.num_heads,self.head_embed_size)
        V_kd = self.v_proj(value).contiguous().view(-1, batch_size,self.num_heads,self.head_embed_size)

        K_kd = K_kd.transpose(0,2).contiguous().view(self.num_heads*batch_size,-1,self.head_embed_size)
        Q_kd = Q_kd.transpose(0,2).contiguous().view(self.num_heads*batch_size,tgt_time_steps,self.head_embed_size)
        V_kd = V_kd.transpose(0,2).contiguous().view(self.num_heads*batch_size,-1,self.head_embed_size)

        # attn_weight = [self.num_heads*batch_size, tgt_time_steps, source_time steps]
        attn_weights = torch.bmm(Q_kd,K_kd.transpose(1,2))/self.head_scaling

        #   Apply masking into att_weights
        if key_padding_mask is not None:
            #   attn_weight size:   [self.num_heads * batch_size, tgt_time_steps, source_time steps] ->
            #                       [batch_size, self.num_heads, tgt_time_steps, source_time steps]
            attn_weights = attn_weights.contiguous().view(batch_size,self.num_heads,tgt_time_steps,-1)
            #   key_padding_mask size   [batch_size,source_time steps] ->
            #                           [ batch_size, 1, 1,source_time steps]
            key_padding_mask = key_padding_mask.unsqueeze(dim=1).unsqueeze(dim=2)
            attn_weights.masked_fill_(key_padding_mask, float('-inf'))
            # transfer back the shape of attn_weight
            attn_weights = attn_weights.contiguous().view(self.num_heads*batch_size, -1, tgt_time_steps)
        if attn_mask is not None:
            #   attn_mask size: [tgt_time_steps, source_time steps]
            #   attn_weight size: [self.num_heads * batch_size, tgt_time_steps, source_time steps]

            #   attn_mask size:     [tgt_time_steps, source_time steps] ->
            #                       [1,tgt_time_steps, source_time steps]
        ...
        # bmm attn_weights  [self.num_heads * batch_size, tgt_time_steps, source_time steps]
        #     V_kd          [self.num_heads*batch_size, source_time steps, self.head_embed_size]
        #       attn [self.num_heads*batch_size, tgt_time_steps, self.head_embed_size]
        attn = torch.bmm(attn_weights, V_kd)
        #   attn ->[tgt_time_steps, batch_size, embed_dim]
        attn = attn.transpose(0, 1).contiguous().view(tgt_time_steps, batch_size, self.num_heads, self.head_embed_size)\
                                                .view(tgt_time_steps, batch_size, embed_dim)
